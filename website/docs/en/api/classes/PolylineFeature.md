...
Polyline element (PolylineFeature).

Implemented based on `PolylineGroundRenderable`, optional ground-clamping, and supports celestial body following (performs local-to-world conversion for non-Earth bodies).
...
Destroy the underlying polyline rendering object and release resources.
...
Get the world coordinate bounding sphere corresponding to the current polyline path.

Participates in Entity aggregation by default; guidelines or temporary effects can
be disabled via `setIncludeInBoundingSphere(false)` or options.includeInBoundingSphere.
...
Register the polyline feature to the entity and create the underlying rendering object when first available.
...
Current polyline feature instance.
...
Directly set the polyline visibility state.
...
Refresh the polyline display state, material, and waypoints at the current time.

For dynamic paths, coordinates are recalculated to ensure polyline endpoints follow the entity in real-time.
