...
Free geometry Feature — allows users to create arbitrary 3D geometry without exposing the underlying layer.

Users pass a DaisyGeometryDescriptor (vertices/normals/indices in local coordinates),
Daisy automatically builds the underlying Daisy.Geometry + SafePrimitive + MaterialAppearance,
and adds it to engine.collections.primitiveCollection.

The geometry follows Entity movement/rotation (local coordinate system), consistent with CylinderFeature/BoxFeature.
...
Replaces geometry at runtime. Triggers primitive rebuild.
...
Replaces material at runtime. Triggers primitive rebuild.
