...
- Optional Worker precomputation to reduce main thread per-frame computation overhead
...
Create a trail path Feature.

Note: Only saves/merges configuration here; the underlying Polyline is not created immediately.
The actual rendering object is added to the collection after register(entity).
...
Destroy and remove the underlying Polyline from the collection.
...
State cleanup during rebind/rebuild.

This clears all keys related to "sampling plan/cache/access range",
to ensure the next update goes through the full computation process instead of reusing old cache.
...
Sync the current color, line width, visibility, and material to the trail line render object.

Note: trail point positions are mainly updated in update(...); reset is more for "style/switch" synchronization.
...
Per-frame update: determines visibility, throttling buckets, and final trail point refresh.

The update logic hierarchy is roughly:
1) Calculate visibility and continuous segment clampAbs
2) Perform "fast throttling" (fastKey) on non-activated entities to reduce meaningless updates
3) Calculate the sampling plan and use intervalSimSecond bucketing (key) to control recalculation frequency
4) When cache hit, only update split and transition segments; otherwise recalculate main trail point series
