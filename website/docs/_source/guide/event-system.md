...
| 层级 | 使用入口 | 适用场景 |
|------|----------|----------|
| [Entity 事件](/guide/entity#事件系统) | `entity.onClick()` / `entity.onUpdate()` 等 | 实体的生命周期、选择和交互 |
| [Feature 事件](/guide/feature#事件系统) | `feature.onClick()` / `feature.onUpdate()` 等 | Feature 粒度的交互（冒泡到 Entity） |
| [物理对象事件](/guide/satellite#事件) | `sat.onClick()` / `sat.onUpdate()` 等 | PW 层对象的交互（桥接到 Entity） |
...
Feature 提供完整的生命周期事件钩子（详见 [Feature 可视化组件](/guide/feature#生命周期详解)）：
