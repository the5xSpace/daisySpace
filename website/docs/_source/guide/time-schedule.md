...
1. **仿真时钟** — Engine 层的时间播放控制（已涵盖在 [Engine 引擎](/guide/engine#时间控制)）
...
>
> **陷阱 3 — `engine.clock` 陷阱的延续**：如果 `engine.clock` 未正确同步（见 [Engine 陷阱](/guide/engine)），调度器从 `engine.getCurrentTime()` 读取的可能是实时时间而非仿真时间，导致任务永远无法匹配。
