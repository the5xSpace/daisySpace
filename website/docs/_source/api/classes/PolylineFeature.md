...
[daisy-space-sdk](../README.md) / PolylineFeature

# Class: PolylineFeature

折线要素（PolylineFeature）。

基于 `PolylineGroundRenderable` 实现，可选贴地渲染，并支持天体跟随（非地球天体时会做 local-to-world 转换）。
...
> **new PolylineFeature**(`options`): `PolylineFeature`
...
> `readonly` `static` **type**: `"PolylineFeature"` = `'PolylineFeature'`
...
> **get** **options**(): [`PolylineOptions`](../types/PolylineOptions.md)
...
`Partial`\<[`PolylineOptions`](../types/PolylineOptions.md)\>
...
默认参与 Entity 聚合；辅助线或临时效果可通过
`setIncludeInBoundingSphere(false)` 或 options.includeInBoundingSphere 关闭。
...
> **register**(`entity`): `PolylineFeature`

将折线要素注册到实体上，并在首次可用时创建底层渲染对象。
...
按当前时刻刷新折线的显示状态、材质和路径点列。

对动态路径会重新计算坐标，确保折线端点能跟随实体实时变化。
