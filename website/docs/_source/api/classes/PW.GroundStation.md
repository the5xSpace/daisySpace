...
GroundStation：地面站点/测站。

语义约定：
- 默认传感器安装方向为 TO_UP（指向天空）
- 业务侧可通过 `addSensor({ emitDirection })` 覆盖安装方向
...
// 关闭：new GroundStation({ useBuiltinModel: false }) 或 model: false
...
> **get** **options**(): [`GroundStationConfig`](../types/PW.GroundStationConfig.md)
...
该方法面向模型节点控制，波束真实指向仍建议由 Sensor 的 link.track 负责。
