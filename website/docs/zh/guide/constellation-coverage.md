...
[Constellation](/api/classes/PW.Constellation) 是卫星的聚合容器，提供批量操作接口。
...
覆盖分析的 `polygons` 输出可通过 [CoverageAreaFeature](/api/classes/CoverageAreaFeature) 渲染：
...
> **相关 API**：[PW.Constellation](/api/classes/PW.Constellation) · [PW.Satellite](/api/classes/PW.Satellite) · [CoverageAreaFeature](/api/classes/CoverageAreaFeature)
