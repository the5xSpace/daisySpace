...
PopoverFeature
DOM 弹出层组件，用于将 HTML 元素绑定到三维实体上，并跟随实体移动。
支持自定义定位方向、样式、固定尺寸及自动显隐。
...
`Partial`\<[`PopoverOptions`](../interfaces/UI.PopoverOptions.md)\>
...
会受 entity show、坐标、距离、遮挡、离屏等运行时条件影响。
...
注意：实际 DOM 是否显示请读取 `renderedVisible`。
...
销毁组件
移除 DOM 元素（或根据 destroyDOM 配置进行隐藏处理）
