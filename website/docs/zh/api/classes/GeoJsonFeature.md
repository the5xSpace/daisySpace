...
[daisy-space-sdk](../README.md) / GeoJsonFeature
...
> **new GeoJsonFeature**(`options?`): `GeoJsonFeature`
...
> `readonly` `static` **type**: `"GeoJsonFeature"` = `"GeoJsonFeature"`
...
> **get** **options**(): [`GeoJsonFeatureOptions`](../interfaces/GeoJsonFeatureOptions.md)
...
`Partial`\<[`GeoJsonFeatureOptions`](../interfaces/GeoJsonFeatureOptions.md)\>
...
> **loadFromGeoJson**(`geoJson`, `callback?`): `void`

直接传入已解析的 GeoJSON 对象并渲染
...
> **loadFromUrl**(`url`, `callback?`): `Promise`\<`void`\>

从 URL 加载 GeoJSON 并渲染
