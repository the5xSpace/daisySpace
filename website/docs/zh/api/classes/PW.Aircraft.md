...
Aircraft：飞行器（Vehicle 的特化）。

语义约定：
- 默认传感器安装方向为 TO_BOTTOM（机体下视）
...
// 默认自动挂载内置 aircraft 模型；useBuiltinModel: false 可关闭
// 默认下视
...
构造期默认挂载内置 aircraft 模型；可通过 `model` 覆盖，
...
添加传感器（并补齐飞行器常见的默认波束长度推断逻辑）。

说明：
- 当未显式给定 `beamLength/range` 时，会尝试从对象状态推断一个合理的默认长度
- 推断逻辑复用 Vehicle 的实现
