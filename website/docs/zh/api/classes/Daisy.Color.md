...
[daisy-space-sdk](../README.md) / [Daisy](../modules/Daisy.md) / Color
...
> `readonly` `static` **DARKGREY**: `Color`
...
> `readonly` `static` **DARKSLATEGREY**: `Color`
...
> `readonly` `static` **LAVENDAR\_BLUSH**: `Color`
...
> `readonly` `static` **LIGHTGREY**: `Color`
...
> `readonly` `static` **LIGHTSLATEGREY**: `Color`
...
> `readonly` `static` **SLATEGREY**: `Color`
...
```ts
const brightBlue = Daisy.Color.BLUE.brighten(0.5, new Daisy.Color());
```
...
```ts
const darkBlue = Daisy.Color.BLUE.darken(0.5, new Daisy.Color());
```
...
```ts
const rgba = Daisy.Color.BLUE.toRgba();
```
...
```ts
const translucentRed = Daisy.Color.RED.withAlpha(0.9);
```
...
```ts
const translucentRed = Daisy.Color.fromAlpha(Daisy.Color.RED, 0.9);
```
...
Creates a Color instance from a [Cartesian4](Daisy.Cartesian4.md). `x`, `y`, `z`,
...
```ts
const cesiumBlue = Daisy.Color.fromCssColorString('#67ADDF');
const green = Daisy.Color.fromCssColorString('green');
```
...
const color = Daisy.Color.fromRandom();

//Create a random shade of yellow.
const color1 = Daisy.Color.fromRandom({
 red : 1.0,
 green : 1.0,
 alpha : 1.0
});
...
const color2 = Daisy.Color.fromRandom({
 minimumRed : 0.75,
 minimumGreen : 0.75,
 minimumBlue : 0.75,
 alpha : 1.0
...
```ts
const color = Daisy.Color.fromRgba(0x67ADDFFF);
```
...
> `static` **lerp**(`start`, `end`, `t`, `result`): `Color`
